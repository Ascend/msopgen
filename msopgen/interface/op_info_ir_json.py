...
class JsonIROpInfo(OpInfo):
    ...
    def _mapping_input_output_type(ir_type: str, ir_name: str) -> any:
        ...
        return utils.CheckFromConfig().trans_io_dtype(ir_type, ir_name, file_type)
    ...
    def _init_param_type(input_output_map: dict, input_output_name: str) -> str:
        ...
        if param_type not in ConstManager.INPUT_OUTPUT_PARAM_TYPE:
            ...
            utils.print_warn_log(
                "The param_type of %s is invalid or None, Assign it the default value \"required\"" % input_output_name
            )
        return param_type
    ...
    def _parse_json_to_info(self: any) -> None:
        utils.print_info_log("Start to parse the ir template:%s" % self.op_path)
        ...
        if input_list is not None:
            ...
            utils.print_warn_log("The \"input_desc\" value is invalid or no \"input_desc\" exists in the map.")
        if output_list is not None:
            self._add_input_output_from_json("output_desc", output_list)
        else:
            utils.print_warn_log("The \"output_desc\" value is invalid or no \"output_desc\" exists in the map.")
        if attr_list is not None:
            self._add_attr_from_json(attr_list)
        else:
            utils.print_warn_log("The \"attr\" value is invalid or no \"attr\" exists in the map.")

    def _check_input_output_info(self: any) -> None:
        if not self.parsed_input_info:
            utils.print_warn_log(
                "There is no input in the IR json. Please "
        ...
        if not self.parsed_output_info:
            utils.print_warn_log(
                "There is no output in the IR json. Please "
                "check the input or output type. If you "
            ...
            if first_count == 0:
                ...
                if ir_type_count != first_count:
                    utils.print_error_log(
                        "The number(%d) of type for \"%s\" is inconsistent with "
                ...
                if format_count != first_count:
                    utils.print_error_log(
                        "The number(%d) of format for \"%s\" is inconsistent with "
                        "the number(%d) of \"%s\" in %s. Please check."
    ...
    def _check_json_data(self: any, json_data: any) -> None:
        if not isinstance(json_data, (list, dict)):
            utils.print_error_log("Data in %s should be List or Dict." % self.op_path)
            raise utils.MsOpGenException(ConstManager.MS_OP_GEN_JSON_DATA_ERROR)
        if isinstance(json_data, list) and len(json_data) < 1:
            utils.print_error_log("There is an operator definition map in %s. Please check." % self.op_path)
            raise utils.MsOpGenException(ConstManager.MS_OP_GEN_JSON_DATA_ERROR)
    ...
    def _read_json_data(self: any, json_data: any) -> dict:
        ...
        for json_map in json_data:
            ...
            if json_map.get("op") is None:
                utils.print_warn_log("The map in %s does not have the \"op\" key.Please check." % self.op_path)
            else:
                ...
                if utils.check_name_valid(op_name) == ConstManager.MS_OP_GEN_NONE_ERROR:
                    ...
                    if op_name in ir_map.keys():  # pylint: disable=consider-iterating-dictionary
                        utils.print_warn_log(
    ...
    def _choose_op_for_generate(self: any, ir_map: dict) -> any:
        ...
        if not ir_info:
            utils.print_error_log("Failed to obtain op info for '%s'. Please check the json." % op_name)
            raise utils.MsOpGenException(ConstManager.MS_OP_GEN_INVALID_SHEET_PARSE_ERROR)
    ...
    def _choose_op(self: any, op_names: list) -> str:
        if self.choose_op != "":
            utils.print_info_log("Start to parse '%s' in the json ir template." % self.choose_op)
            if self.choose_op not in op_names:
                utils.print_error_log(
                    "Failed to find '%s' in json. Please check that the value for '-op' is valid." % self.choose_op
                )
                raise utils.MsOpGenException(ConstManager.MS_OP_GEN_INVALID_PARAM_ERROR)
            return self.choose_op
        if len(op_names) > 1:
            utils.print_info_log("There is more than one operator in the .json file:")
            i = 1
            for op_name in op_names:
                print(i, op_name)
                i += 1
            while True:
                ...
                if op_number.isdigit():
                    ...
                    if op_number < 1 or op_number > len(op_names):
                        utils.print_warn_log("The input is out of range, please retype one.")
                    else:
                        op_name = op_names[op_number - 1]
                        utils.print_info_log("You have chosen: " + op_name)
                        return op_name
                else:
                    utils.print_warn_log("The input is not a number, please retype!")
        elif len(op_names) == 0:
    ...
    def _check_type_format_length(self, check_map: dict) -> dict:
        ...
        if type_length == format_length:
            return check_map
        utils.print_warn_log(
            "the number of formats and types of %s is different, "
        ...
        if len(format_list) == 1:
            ...
            utils.print_error_log("invalid number count for type and format in %s, please check." % input_output_name)
            raise utils.MsOpGenException(ConstManager.MS_OP_GEN_JSON_DATA_ERROR)
        return check_map
    ...
    def _update_input_output_info(self: any, prefix: str, input_output_map: any) -> None:
        if isinstance(input_output_map, dict):
            ...
            if input_output_name is None or not isinstance(input_output_name, str):
                utils.print_warn_log("The input or output name is None or invalid. please check.")
                return
            input_output_name = input_output_name.strip()
            if utils.check_cpp_identifier_valid(input_output_name) != ConstManager.MS_OP_GEN_NONE_ERROR:
                utils.print_error_log(
                    "The input or output name '%s' is not a valid identifier "
                    "for code generation. Only alphanumeric characters and "
                    "underscores are allowed." % input_output_name
                )
                raise utils.MsOpGenException(ConstManager.MS_OP_GEN_INVALID_PARAM_ERROR)
            types = input_output_map.get("type")
            if types is None or not isinstance(types, (list, str)):
                utils.print_warn_log("The input or output type is None or invalid. please check.")
                return
            ...
            if self.gen_flag and self.framework not in ConstManager.FMK_MS:
                ...
                if formats is None or not isinstance(formats, (list, str)):
                    utils.print_warn_log("The input or output format is None or invalid. please check.")
                    return
            ...
            op_format = self._init_op_format(input_output_map, prefix, input_output_name, [ir_type_list, types])
            param_type = self._init_param_type(input_output_map, input_output_name)
            self._update_parsed_info(prefix, input_output_name, ir_type_list, [param_type, op_format])
        else:
            utils.print_warn_log("Every value in the \"%s\" list should be dict" % prefix)

    def _init_ir_type(self: any, prefix: str, input_output_name: any, types: list) -> list:
        ir_type_list = []
        for ir_type in types:
            converted_type = self._mapping_input_output_type(ir_type.strip(), input_output_name)
            if converted_type:
                ir_type_list += converted_type.split(",")
        if not ir_type_list:
            utils.print_warn_log("The %s ir type is invalid: %s" % (prefix, types))
            utils.print_error_log(
                "The input or output type in the json file is not supported. Please check the input or output type."
            )
            raise utils.MsOpGenException(ConstManager.MS_OP_GEN_INVALID_PARAM_ERROR)
        return ir_type_list

    def _init_op_format(self, input_output_map: dict, prefix: str, input_output_name: str, types_list: list) -> any:
        ...
        if type_sum > len(types):
            utils.print_warn_log(
                "The number of formats does not match that of types, which will be automatically "
        ...
        utils.print_error_log(
            "The number of types does not match that of formats for \"%s\" in %s. Please check."
            % (input_output_name, self.op_path)
        )
        raise utils.MsOpGenException(ConstManager.MS_OP_GEN_INCONSISTENT_NUMBER)

    def _update_parsed_info(
        self: any, prefix: str, input_output_name: str, ir_type_list: list, type_format: list
    ) -> None:
        ...
        if prefix == "input_desc":
            if input_output_name in self.parsed_input_info:
                utils.print_warn_log(
                    "The input name \"%s\" is duplicate.  The last one is to be used!" % input_output_name
                )
            self.parsed_input_info.update(
                {
                    input_output_name: {
            ...
            if input_output_name in self.parsed_output_info:
                utils.print_warn_log(
                    "The out name \"%s\" is duplicate.  The last one is to be used!" % input_output_name
                )
            self.parsed_output_info.update(
                {
                    input_output_name: {
                        ConstManager.INFO_IR_TYPES_KEY: ir_type_list,
                        ConstManager.INFO_PARAM_TYPE_KEY: param_type,
    ...
    def _update_attr_info(self: any, attr_map: dict) -> None:
        ...
        if attr_name is None or not isinstance(attr_name, str):
            utils.print_warn_log("The attr_name name is None or invalid. Please check!")
            return
        attr_name = attr_name.strip()
        if utils.check_cpp_identifier_valid(attr_name) != ConstManager.MS_OP_GEN_NONE_ERROR:
            utils.print_error_log(
                "The attribute name '%s' is not a valid identifier "
                "for code generation. Only alphanumeric characters and "
                "underscores are allowed." % attr_name
            )
            raise utils.MsOpGenException(ConstManager.MS_OP_GEN_INVALID_PARAM_ERROR)
        op_type = attr_map.get("type")
        if op_type is None or not isinstance(op_type, str):
            utils.print_warn_log("The op_type name is None or invalid. Please check!")
            return
        ...
        if utils.check_cpp_value_valid(default_value) != ConstManager.MS_OP_GEN_NONE_ERROR:
            utils.print_error_log(
                "The default value of attribute '%s' contains unsafe characters for code generation." % attr_name
            )
            raise utils.MsOpGenException(ConstManager.MS_OP_GEN_INVALID_PARAM_ERROR)
        ...
        if param_type not in ConstManager.ATTR_PARAM_TYPE:
            param_type = ConstManager.PARAM_TYPE_REQUIRED
            utils.print_warn_log(
                "The param_type of %s is invalid or None. Assign it the default value \"required\"" % attr_name
            )
        self.parsed_attr_info.append([attr_name, attr_type, default_value, param_type])
