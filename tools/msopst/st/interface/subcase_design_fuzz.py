...
class SubCaseDesignFuzz(SD.SubCaseDesign):
    ...
    def _get_fuzz_func(module_name, fuzz_func, real_fuzz_path):
        spec = importlib.util.spec_from_file_location(module_name, real_fuzz_path)
    ...
    def subcase_generate(self):
        ...
        prefix = '{}{}'.format(self.json_obj.get(ConstManager.CASE_NAME).replace('/', '_'), '_fuzz_case_')
        ...
        for _ in range(loop_num):
            ...
            for fuzz_dict in fuzz_return_list:
                if ori_json.get(ConstManager.ST_MODE) == "ms_python_train":
                    input_desc_list = self._make_desc_list_ms_fuzz(ori_json, fuzz_dict, ConstManager.INPUT_DESC)
                    output_desc_list = self._make_desc_list_ms_fuzz(ori_json, fuzz_dict, ConstManager.OUTPUT_DESC)
                else:
                    input_desc_list = self._make_desc_list_fuzz(ori_json, fuzz_dict, ConstManager.INPUT_DESC)
                    output_desc_list = self._make_desc_list_fuzz(ori_json, fuzz_dict, ConstManager.OUTPUT_DESC)
                ...
                if ori_json.get(ConstManager.ST_MODE) == "ms_python_train":
                    ...
                    suffix = '_'.join(suffix_list)
                case = {
                    ConstManager.OP: ori_json[ConstManager.OP],
        ...
        utils.print_info_log(
            'Create %d fuzz test cases for %s.'
            % (loop_num * len(fuzz_return_list) - repeat_case_num, self.json_obj[ConstManager.CASE_NAME])
        )
    ...
    def check_fuzz_valid(self, json_obj):
        ...
        if len(fuzz_impl_path_func.split(":")) == 1:
            ...
            utils.print_error_log(
                'The fuzz file "%s" is invalid, ex: fuzz.py:fuzz_function.Please modify it.' % fuzz_impl_path_func
            )
        ...
        if os.path.splitext(real_fuzz_path)[-1] != ConstManager.PY_FILE:
            utils.print_error_log(
                'The fuzz file "%s" is invalid, only supports .py file. Please modify it.' % fuzz_impl_path
            )
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_INVALID_PATH_ERROR)
        ...
        utils.print_info_log("Start to import %s in %s." % (module_name, real_fuzz_path))
        try:
            fuzz_function = self._get_fuzz_func(module_name, fuzz_func, real_fuzz_path)
        except Exception as ex:
            utils.print_error_log('Failed to execute function "%s" in %s. %s' % (fuzz_func, fuzz_file, str(ex)))
    ...
    def _check_fuzz_case_num_valid(self, json_obj):
        ...
        if isinstance(fuzz_case_num, int):
            ...
            utils.print_error_log(
                ...
                'Please modify it.'
                % (ConstManager.FUZZ_CASE_NUM, self.current_json_path, ConstManager.MAX_FUZZ_CASE_NUM)
            )
    ...
    def _check_fuzz_value_valid(self, json_tuple, param_type, fuzz_dict, required=True):
        json_obj, key, support_list = json_tuple
        if required:
            self._check_key_exist(json_obj, key, param_type)
        json_obj = self._replace_fuzz_param(json_obj, key, param_type, fuzz_dict)
    ...
    def _check_fuzz_shape_valid(self, json_tuple, param_type, fuzz_dict, required=True):
    ...
    def _check_fuzz_data_distribute_valid(self, json_obj, key, param_type, fuzz_dict):
        if key in json_obj and ConstManager.VALUE not in json_obj:
            data_distribute_value = self._check_fuzz_value_valid(
                (json_obj, key, self.WHITE_LISTS.data_distribution_list), param_type, fuzz_dict
            )
        else:
            data_distribute_value = 'uniform'
        return data_distribute_value

    def _check_fuzz_value_range_valid(self, json_obj, key, param_type, fuzz_dict):
        if key in json_obj and ConstManager.VALUE not in json_obj:
            self._check_key_exist(json_obj, key, param_type)
            json_obj = self._replace_fuzz_param(json_obj, key, param_type, fuzz_dict)
    ...
    def _make_desc_list_ms_fuzz(self, json_obj, fuzz_dict, desc_type):
        ...
        if len(json_obj[desc_type]) == 0:
            utils.print_error_log(
                'The value of "%s" is empty. Please modify it in file %s.' % (desc_type, self.current_json_path)
            )
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_INVALID_DATA_ERROR)
        for desc_obj in json_obj[desc_type]:
            type_value = self._check_fuzz_value_valid(
                (desc_obj, 'type', self.WHITE_LISTS.mindspore_type_list), desc_type, fuzz_dict
            )
            shape_value = self._check_fuzz_shape_valid((desc_obj, 'shape'), desc_type, fuzz_dict)
            if desc_type == ConstManager.INPUT_DESC:
                ...
                value_range = self._check_fuzz_value_range_valid(desc_obj, 'value_range', desc_type, fuzz_dict)
                one_desc = {
                    'type': type_value,
                    'shape': shape_value,
                ...
                desc_obj = self._replace_fuzz_param(desc_obj, ConstManager.VALUE, desc_type, fuzz_dict)
                self._deal_with_value(desc_obj, one_desc, for_fuzz=True)
            else:
                one_desc = {'type': type_value, 'shape': shape_value}
            desc_list.append(one_desc)
        return desc_list

    def _deal_with_ori_filed_data_fuzz(self, json_obj, param_type, fuzz_dict, one_input_desc):
    ...
    def _check_fuzz_shape_range_valid(self, json_obj, key, param_type, fuzz_dict):
        json_obj = self._replace_fuzz_param(json_obj, key, param_type, fuzz_dict)
    ...
    def _deal_with_dynamic_shape_fuzz(self, json_obj, param_type, fuzz_dict, one_input_desc):
        if not dynamic_handle.check_not_dynamic_shape(json_obj.get('shape')):
            return one_input_desc
        typical_shape = self._check_fuzz_shape_valid((json_obj, ConstManager.TYPICAL_SHAPE), param_type, fuzz_dict)
        dynamic_handle.check_typical_shape_valid(typical_shape, self.current_json_path)
        one_input_desc[ConstManager.TYPICAL_SHAPE] = typical_shape
        shape_range = self._check_fuzz_shape_range_valid(json_obj, ConstManager.SHAPE_RANGE, param_type, fuzz_dict)
    ...
    def _make_desc_list_fuzz(self, json_obj, fuzz_dict, desc_type):
        ...
        if len(json_obj[desc_type]) == 0:
            if desc_type == ConstManager.INPUT_DESC:
                utils.print_warn_log('The value of "input_desc" is empty.')
                return desc_list
            utils.print_error_log(
                'The value of "%s" is empty. Please modify it in file %s.' % (desc_type, self.current_json_path)
            )
        ...
        for desc_obj in json_obj[desc_type]:
            format_value = self._check_fuzz_value_valid(
                (desc_obj, 'format', list(self.WHITE_LISTS.format_map.keys())), desc_type, fuzz_dict
            )
            type_value = self._check_fuzz_value_valid(
                (desc_obj, 'type', self.WHITE_LISTS.type_list), desc_type, fuzz_dict
            )
            shape_value = self._check_fuzz_shape_valid((desc_obj, 'shape'), desc_type, fuzz_dict)
            if desc_type == ConstManager.INPUT_DESC:
                data_distribute = self._check_fuzz_data_distribute_valid(
                ...
                one_desc = {'format': format_value, 'type': type_value, 'shape': shape_value}
            # check whether has ori_format and ori_shape.
            one_desc = self._deal_with_ori_filed_data_fuzz(desc_obj, desc_type, fuzz_dict, one_desc)
            # check whether the shape is dynamic.
            if desc_obj.get(ConstManager.TYPICAL_SHAPE) is not None:
                one_desc = self._deal_with_dynamic_shape_fuzz(desc_obj, desc_type, fuzz_dict, one_desc)
