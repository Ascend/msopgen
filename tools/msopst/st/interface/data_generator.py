...
class DataGenerator:
    ...
    def __init__(self, case_list, output_path, cmd_mi, report):
        ...
        self.cmd_mi = cmd_mi
        if cmd_mi:
            self.output_path = os.path.join(output_path, 'run', 'out', 'test_data', 'data')
        else:
            op_name_path = os.path.join(output_path, case_list[0]['op'])
            self.output_path = os.path.join(output_path, op_name_path, 'run', 'out', 'test_data', 'data')
    ...
    def gen_data(data_shape, min_value, max_value, dtype, distribution='uniform'):
        ...
        if distribution == 'uniform':
            ...
            data = np.random.uniform(low=min_value, high=max_value, size=data_shape).astype(np_type)
            ...
            data = np.random.normal(loc=min_value, scale=abs(max_value) + 1e-4, size=data_shape).astype(np_type)
            ...
            data = np.random.beta(a=abs(min_value) + 1e-4, b=abs(max_value) + 1e-4, size=data_shape).astype(np_type)
            ...
            data = np.random.laplace(loc=min_value, scale=abs(max_value) + 1e-4, size=data_shape).astype(np_type)
            ...
            data = np.random.triangular(left=min_value, mode=mode, right=max_value, size=data_shape).astype(np_type)
        elif distribution == 'relu':
            # Returns the random value after the uniform distribution
            # and relu activation.
            data_pool = np.random.uniform(low=min_value, high=max_value, size=data_shape).astype(np_type)
            ...
            # and tanh activation.
            data_pool = np.random.uniform(low=min_value, high=max_value, size=data_shape).astype(np_type)
            data = (np.exp(data_pool) - np.exp(-data_pool)) / (np.exp(data_pool) + np.exp(-data_pool))
        else:
            utils.print_error_log('The distribution(%s) is invalid.' % distribution)
    ...
    def generate(self):
        ...
        for case in self.case_list:
            ...
            utils.print_info_log('Start to generate the input data for %s.' % case_name)
            param_info = ""
            # get intput  and output param
            param_info_list, calc_func_params_tmp = self._generate_params_desc(case, case_name)
            # get attr param
            if case.get('attr'):
                for _, attr in enumerate(case.get('attr')):
                    attr_name = attr.get('name')
                    param_info_list.append("{attr_name}".format(attr_name=attr_name))
                    calc_func_params_tmp.update({attr_name: attr.get('value')})
            if case.get("calc_expect_func_file") and case.get("calc_expect_func_file_func"):
                ...
                utils.print_info_log("Please ensure that the above parameters in the expected function are consistent.")
                utils.print_info_log(
                    '------------------------------------------------------------------------------------------------'
                )
            expect_data_paths = self._generate_expect_data(case, calc_func_params_tmp)
            # deal with report
            case_report = self.report.get_case_report(case_name)
            case_report.trace_detail.st_case_info.input_data_paths = self.output_path
            if expect_data_paths:
                case_report.trace_detail.st_case_info.expect_data_paths = expect_data_paths
                utils.print_info_log('Finish to generator the expect output data for %s.' % case_name)
    ...
    def gen_data_for_ascendc(self, output_path):
        ...
        for case in self.case_list:
            ...
            for index, input_desc in enumerate(case.get('input_desc')):
                ...
                input_shape = dynamic_handle.replace_shape_to_typical_shape(input_desc)
    ...
    def _gen_op_iput_data(self, input_shape, input_desc):
        ...
        if value:
            ...
            data = self.gen_data(input_shape, range_min, range_max, dtype, input_desc.get('data_distribute'))
        return data
    ...
    def _gen_input_data(self, input_shape, input_desc, file_path):
        try:
            ...
            utils.print_warn_log(
                'Failed to generate data for %s. The shape is too large to invoke MemoryError. %s' % (file_path, error)
            )
    ...
    def _get_input_dict_with_data(self, input_desc, file_path, input_shape):
        ...
        """
        is_const_distribute = (
            input_desc.get(ConstManager.IS_CONST)
            and input_desc.get(ConstManager.DATA_DISTRIBUTE)
            and input_desc.get(ConstManager.VALUE) is None
        )
    ...
    def _get_trans_data_input_dict(self, input_desc, file_path, input_shape):
        if os.path.exists(file_path):
            utils.print_error_log('The file %s already exists, please delete it then retry.' % file_path)
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_WRITE_FILE_ERROR)
        ...
        try:
            ...
            utils.print_warn_log('Failed to generate data for %s. %s' % (file_path, error))
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_WRITE_FILE_ERROR) from error
        finally:
            pass
        return input_dic

    def _get_input_desc_and_gen_data(self, case, case_name, calc_func_params_tmp, param_info_list):
        """
        get input desc info
        """
        for index, input_desc in enumerate(case.get('input_desc')):
            ...
            # consider dynamic shape scenario
            input_shape = dynamic_handle.replace_shape_to_typical_shape(input_desc)
            file_path = os.path.join(self.output_path, case_name + '_input_' + str(index) + '.bin')
            ...
            if input_desc.get('name'):
                ...
                calc_func_params_tmp.update({input_name: input_dict})
                param_info_list.append("{input_name}".format(input_name=input_name))

    def _generate_params_desc(self, case, case_name):
        calc_func_params_tmp = {}
        param_info_list = []
        self._get_input_desc_and_gen_data(case, case_name, calc_func_params_tmp, param_info_list)
        for _, output_desc in enumerate(case.get('output_desc')):
            output_shape = dynamic_handle.replace_shape_to_typical_shape(output_desc)
            output_dic = {'dtype': output_desc.get('type'), 'shape': output_shape, 'format': output_desc.get('format')}
            if output_desc.get('name'):
                output_name = output_desc.get('name')
                calc_func_params_tmp.update({output_name: output_dic})
                param_info_list.append("{output_name}".format(output_name=output_name))
    ...
    def _get_tensors_and_func(self, case, calc_func_params_tmp):
        ...
        utils.print_info_log("Start to import %s in %s." % (module_name, py_file))
        spec = importlib.util.spec_from_file_location(module_name, expect_func_file)
        ...
        try:
            ...
            utils.print_error_log(
                'Failed to execute function "%s" in %s. %s' % (expect_func, expect_func_file, str(ex))
            )
    ...
    def _generate_expect_data(self, case, calc_func_params_tmp):
        ...
        if case.get("calc_expect_func_file") and case.get("calc_expect_func_file_func"):
            utils.print_info_log('Start to generate the expect output data for %s.' % case_name)
            expect_result_tensors, expect_func = self._get_tensors_and_func(case, calc_func_params_tmp)
            for idx, expect_result_tensor in enumerate(expect_result_tensors):
                ...
                if str(expect_result_tensor.dtype) != output_dtype:
                    utils.print_warn_log(
                        "The dtype of expect date clc by "
                        "%s is %s , is not same as "
                        "the dtype(%s) in output_desc index("
                        "%s). " % (expect_func, expect_result_tensor.dtype, output_dtype, str(idx))
                    )
                expect_data_name = "%s_expect_output_%s_%s.bin" % (case.get('case_name'), str(idx), output_dtype)
                expect_data_path = os.path.join(expect_data_dir, expect_data_name)
                expect_result_tensor.tofile(expect_data_path)
                utils.print_info_log("Successfully generated expect data:%s." % expect_data_path)
