...
class CaseGenerator:
    ...
    def check_required_key(op_info, op_info_key, missing_keys):
        ...
        if len(missing_keys) > 0:
            utils.print_error_log('The "%s" is missing: %s. Please modify it.' % (op_info_key, missing_keys))
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
    ...
    @staticmethod
    def _get_ms_ops_info(module_name, class_name, module_file=None):
        if module_file:
    ...
    def _get_mindspore_op_dtype(mindspore_ops_info, count_input):
        ...
        for value in mindspore_ops_info.get('dtype_format'):
            if not value[count_input]:
                utils.print_error_log('The dtype_format of this opeartor is null, please modify it')
    ...
    def _check_op_info_list_valid(value_list, support_list, op_info_key):
        if len(value_list) == 0:
            utils.print_error_log('The value of "%s" is empty. Please modify it.' % op_info_key)
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
        for value in value_list:
            if value == '':
                utils.print_error_log(
                    'The value of "%s" is empty. Only supports %s. Please modify it.' % (op_info_key, support_list)
                )
                raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
            if value not in support_list:
                utils.print_warn_log(
                    'The value(%s) of "%s" is invalid. '
                    'Only supports %s. Please modify it.' % (value, op_info_key, support_list)
                )
    ...
    def _check_op_proto_path_valid(path):
        if not os.path.exists(path):
            utils.print_warn_log('The path {} does not exist. Please check whether the path exists.'.format(path))
            return False
    ...
    def generate(self):
        ...
        try:
            with os.fdopen(os.open(json_path, ConstManager.WRITE_FLAGS, ConstManager.WRITE_MODES), 'w+') as file_object:
                file_object.write(json.dumps(base_case, sort_keys=False, indent=4))
        except IOError as io_error:
            utils.print_error_log('Failed to generate file %s. %s' % (json_path, str(io_error)))
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_WRITE_FILE_ERROR) from io_error
        finally:
            pass
        utils.print_info_log("Generate test case file %s successfully." % json_path)
    ...
    def _get_attr_type_list_value(self, attr_type, default_value_str):
        ...
        if attr_type == 'listListInt':
            default_value = self._parse_list_list_int_value(default_value_str)
    ...
    def _get_default_attr_value(self, attr_type, default_value_str, attr_name):
        ...
        try:
            ...
            utils.print_warn_log(
                ...
                'the default value in "%s" attr. %s' % (default_value_str, attr_type, attr_name, ex)
            )
        finally:
            pass
        return default_value
    ...
    def _get_op_info(self, index, line):
        ...
        if len(key_value) != 2:
            utils.print_error_log(
                ...
                'supports "xx.yy=zz". Please modify it.' % (index, line, self.input_file_path)
            )
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
    ...
    def _get_tbe_ops_info(self, line, tbe_ops_info, index):
        if line.endswith("]"):
            ...
            utils.print_error_log(
                'At line %d, "%s" is invalid in file %s, only '
                'supports "[xx]". Please modify it.' % (index, line, self.input_file_path)
            )
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)

    def _parse_ini_to_json(self):
        tbe_ops_info = {}
        with open(self.input_file_path) as ini_file:  # pylint: disable=unspecified-encoding
        ...
        if len(tbe_ops_info) != 1:
            utils.print_error_log(
                ...
                'in .ini file. Please modify it.' % (len(tbe_ops_info), self.input_file_path)
            )
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
    ...
    def _parse_py_to_json(self):
        ...
        utils.print_info_log("Start to import {} in {}.".format(module_name, py_file))
        class_name = "{}op_info".format(module_name.rstrip("impl"))
        try:
            mindspore_ops_info = self._get_ms_ops_info(module_name, class_name, module_file=expect_func_file)
    ...
    def _parse_cxx_to_json(self):
        ...
        explicit_in_info = op_info_string.__contains__("public:explicit")  # pylint: disable=unnecessary-dunder-call
        name_list = (
            re.findall(r'public:explicit([a-zA-Z0-9_]+)\(', op_info_string)
            if explicit_in_info
    ...
    def _parse_attr_info(self, op_info):
        ...
        if attr_param_type:
            ...
            default_value = None
        attr_type_transform = ''
        if attr_type and attr_type[0] in ConstManager.OP_PROTO_PARSE_ATTR_TYPE_MAP.keys():  # pylint: disable=consider-iterating-dictionary
        ...
        for i_o_type in ConstManager.IO_TYPE:
            ...
            for i_o_desc in op_info:
                if not i_o_desc.get('name'):
                    utils.print_error_log('This %s of this operator is null, please modify it.' % i_o_type)
                    raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_INVALID_DATA_ERROR)
                ...
                self.op_info[op_key]['dtype'] = self._get_mindspore_op_dtype(mindspore_ops_info, count_input)
                count_input += 1

    def _check_basic_filed_vaild(self, dtype_count, op_info, op_info_key):
        # check paramType valid
        self._check_op_info_list_valid(
            [op_info["paramType"]], ConstManager.PARAM_TYPE_VALID_VALUE, op_info_key + '.paramType'
        )
        ...
        if 'dtype' in op_info:
            ...
            if self.input_file_path.endswith(".py"):
                self._check_op_info_list_valid(dtype_list, self.WHITE_LISTS.mindspore_type_list, op_info_key + '.dtype')
            else:
                self._check_op_info_list_valid(dtype_list, self.WHITE_LISTS.type_list, op_info_key + '.dtype')
        ...
        if 'format' in op_info:
            ...
            self._check_op_info_list_valid(
                format_list, list(self.WHITE_LISTS.format_map.keys()), op_info_key + '.format'
            )
            ...
            if current_dtype_count != len(format_list):
                utils.print_error_log(
                    ...
                    % (current_dtype_count, op_info_key, len(format_list), op_info_key)
                )
                raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
        else:
            op_info["format"] = ''
    ...
    def _check_op_info(self):
        ...
        dtype_count = 0
        for op_info_key in self.op_info:  # pylint: disable=consider-using-dict-items
            if op_info_key.startswith(ConstManager.INI_INPUT) or op_info_key.startswith(ConstManager.INI_OUTPUT):
                op_info = self.op_info[op_info_key]
                missing_keys = []
                # check required key is missing
                self.check_required_key(op_info, op_info_key, missing_keys)
                # check paramType/dtype/format valid.
    ...
    def _make_attr(self, key, value):
        ...
        if not self._is_aicpu_op():
            if 'type' not in value:
                utils.print_error_log('The "%s" is missing "type". Please modify it.' % key)
                raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
            self._check_op_info_list_valid([value.get('type')], list(ConstManager.ATTR_TYPE_MAP.keys()), key + '.type')
        ...
        if 'defaultValue' in value and data_type is not None:
            default_value = self._get_default_attr_value(value.get('type'), value.get('defaultValue'), name)
        return {'name': name, 'type': data_type, 'value': default_value}

    def _check_desc_valid(self, base_case, key):
        if len(base_case[key]) == 0:
            utils.print_error_log(
                'The number of %s is zero in file %s. Please modify it.' % (key[:-5], self.input_file_path)
            )
            raise utils.OpTestGenException(ConstManager.OP_TEST_GEN_CONFIG_INVALID_OPINFO_FILE_ERROR)
    ...
    def _parse_name_from_op_proto_file(self, op_key, start_str):
        ...
        try:
            ...
            utils.print_warn_log("Can't parse operator information of %s in %s,please check." % (op_key, head_file))
        finally:
            pass
        return ""
    ...
    def _get_aicpu_op_info_from_ini_file(self):
        ...
        for key, _ in list(self.op_info.items()):
            if key.startswith(ConstManager.INI_INPUT) or key.startswith(ConstManager.INI_OUTPUT):
                ...
                self._check_op_info_list_valid(
                    tensor_type, list(self.WHITE_LISTS.aicpu_ir2ini_type_map.keys()), key + '.type'
                )
                if tensor_type:
    ...
    def _get_op_info_from_head_file(self, op_info_key, input_tensor_type, name):
        ...
        if op_info_key:
            input_tensor_type = input_tensor_type.replace(ConstManager.NEW_LINE_MARK, ConstManager.EMPTY)
            input_tensor_type = input_tensor_type.replace(ConstManager.SPACE, ConstManager.EMPTY)
            dtype_list = list(set(input_tensor_type.split(ConstManager.COMMA)))
            self._check_op_info_list_valid(
                dtype_list, list(self.WHITE_LISTS.aicpu_ir2ini_type_map.keys()), ConstManager.INI_INPUT + '.type'
            )
            self.op_info[op_info_key]['name'] = name
            self.op_info[op_info_key]['type'] = input_tensor_type.replace(' ', '')
        ...
        for op_key in iter(ConstManager.IN_OUT_OP_KEY_MAP):
            if line.startswith(op_key):
                ...
                if not name:
                    break
                # match type.
                input_tensor_type = start_str[start_str.find("{") + 1 : start_str.find("}")]

                op_info_key = ''
                # record the input information of key
                if ConstManager.IN_OUT_OP_KEY_MAP.get(op_key) == 'input':
                    op_info_key = '%s%s' % (ConstManager.IN_OUT_OP_KEY_MAP.get(op_key), input_count)
                    input_count += 1
                # record the output information of key
                if ConstManager.IN_OUT_OP_KEY_MAP.get(op_key) == 'output':
                    op_info_key = '%s%s' % (ConstManager.IN_OUT_OP_KEY_MAP.get(op_key), output_count)
                    output_count += 1
    ...
    def _get_attr_tensor_type(self, op_key, type_attrs, attr_name):
        ...
        if op_key == 'ATTR':
            ...
            try:
                ...
                utils.print_warn_log("Can't parse operator information of %s, please check." % attr_name)
                default_value_str = {}
            finally:
                pass
            format_default_value = utils.format_dict_to_list(default_value_str)
            default_value = format_default_value.replace(ConstManager.NEW_LINE_MARK, ConstManager.EMPTY)
            self.op_info[attr_name]['defaultValue'] = default_value.replace(
    ...
    def _parse_aicpu_attr(self, line):
        for op_key in ConstManager.AICPU_ATTR_LIST:
            if line.startswith(op_key):
                ...
                self._check_op_info_list_valid(
                    [attr_tensor_type], list(ConstManager.OP_PROTO_PARSE_ATTR_TYPE_MAP.keys()), 'attr_' + name + '.type'
                )
                self.op_info[attr_name]['type'] = ConstManager.OP_PROTO_PARSE_ATTR_TYPE_MAP.get(attr_tensor_type)
        ...
        if op_name_h_text:
            ...
            if reg_op_info_str.startswith('REG_OP'):
                ...
                for line in line_point_list:
                    ...
                    input_count, output_count = self._parse_aicpu_input_output_info(line, input_count, output_count)
    ...
    def _find_aicpu_op_proto_path(self):
        ...
        op_proto_path = os.path.realpath(os.path.join(os.path.dirname(self.input_file_path), '../../../op_proto'))
        op_proto_file_name = utils.fix_name_lower_with_under(self.op_type) + '.h'
        ...
        if is_file_exist:
            utils.print_info_log("Start parsing %s to obtain operator information." % op_proto_file_path)
        ...
        if not is_file_exist or not is_parse_success:
            utils.print_warn_log("There are unavailable operator information in %s." % op_proto_file_path)
    ...
    def _generate_aicpu_op_desc(self, value, base_case, op_key):
        ...
        if value.get('format') is None or len(value.get('format')) == 0:
            ...
            format_value = value.get('format').replace(ConstManager.QUOTATION_MARK, ConstManager.EMPTY)
        ...
        if value.get('type') is None or len(value.get('type')) == 0:
            ...
            for dtype_key in dtype_list:
                if dtype_key in self.WHITE_LISTS.aicpu_ir2ini_type_map.keys():
                    trans_dtype_list.append(self.WHITE_LISTS.aicpu_ir2ini_type_map.get(dtype_key))
        ...
        if op_key == 'output_desc':
            op_desc = {'format': op_format, 'type': op_dtype, 'shape': []}
    ...
    def _generate_aicpu_base_case(self):
        ...
        base_case = {
            'case_name': 'Test_%s_001' % self.op_type.replace('/', '_'),
            'op': self.op_type,
            'input_desc': [],
            'output_desc': [],
        }
        for key, value in list(self.op_info.items()):
            if key.startswith(ConstManager.INI_INPUT):
                ...
                self._generate_aicpu_op_desc(value, base_case, 'output_desc')
    ...
    def _generate_input_desc(self, value, base_case):
        ...
        if self.input_file_path.endswith(".py"):
            input_desc = {'type': input_dtype, 'shape': [], 'data_distribute': ['uniform'], 'value_range': [[0.1, 1.0]]}
        else:
            input_desc = {
                'format': input_format,
                'type': input_dtype,
    ...
    def _generate_output_desc(self, value, base_case):
        ...
        if self.input_file_path.endswith(".py"):
            output_desc = {'type': output_dtype, 'shape': []}
        else:
            output_desc = {'format': output_format, 'type': output_dtype, 'shape': []}
    ...
    def _update_aicpu_io_from_model(self, node):
        ...
        new_base_case = {'case_name': 'Test_' + layer_name, 'op': self.op_type, 'input_desc': [], 'output_desc': []}
        ...
        for index, dtype in enumerate(node.get('output_dtype')):
            output_desc = {'format': ['ND'], 'type': dtype, 'shape': node.get('output_shape')[index]}
    ...
    def _get_new_base_case(self, base_case, node):
        layer_name = node.get('layer').replace('/', '_')
        new_base_case = {}
        if len(node.get('input_shape')) != len(base_case.get('input_desc')):
            utils.print_warn_log(
                ...
                % (
                    layer_name,
                    len(node.get('input_shape')),
                    len(base_case.get('input_desc')),
                )
            )
            return new_base_case
        if len(node.get('output_shape')) != len(base_case.get('output_desc')):
            utils.print_warn_log(
                'The "%s" layer is skipped, because its number of '
        ...
        if node.get('attr'):
            for item in node.get('attr'):
                if item.get('name') == "data_format":
                    ...
                    self._update_format_from_model(format_value, new_base_case)
    ...
    def _get_info_from_model(self, item, attr, new_attr_list, new_base_case):
        ...
        if node_attr_name == base_attr_name:
            ...
            if attr.get('type') == 'string':
                attr_format_value = item.get('value').replace(ConstManager.QUOTATION_MARK, ConstManager.EMPTY)
                attr_value = attr_format_value.replace(ConstManager.SPACE, ConstManager.EMPTY)
            new_attr = {'name': node_attr_name, 'type': attr['type'], 'value': attr_value}
    ...
    def _update_aicore_attr_from_model(self, base_case, node, new_base_case):
        if 'attr' in base_case and 'attr' in node:
            ...
            for _, attr in enumerate(base_case.get('attr')):
                for item in node.get('attr'):
                    self._get_info_from_model(item, attr, new_attr_list, new_base_case)
    ...
    def _generate_base_case_from_model(self, base_case, is_aicpu=False):
        ...
        if len(nodes) == 0:
            utils.print_warn_log(
                ...
                "operator info from the model. Please check the model." % self.op_type
            )
            utils.print_info_log("Continue generating the case json file based on the .ini file.")
            return [base_case]
        base_case_list = []
        for _, node in enumerate(nodes):
            ...
            if is_aicpu:
                ...
                new_base_case = self._update_aicore_io_from_model(base_case, node)
                if new_base_case:
                    self._update_aicore_attr_from_model(base_case, node, new_base_case)
                    base_case_list.append(new_base_case)
        if not base_case_list:
            utils.print_warn_log(
                "No match for the layer in the model. Failed "
