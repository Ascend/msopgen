...
class TestUtilsMethods(unittest.TestCase):
    ...
    # ========== cross-module-001 fix: sys.path 不被污染 ==========

    def test_get_ms_ops_info_no_syspath_pollution(self):
        """_get_ms_ops_info 使用 module_file 时不应修改 sys.path"""
        original_path = sys.path.copy()
        # 创建一个简单的 mock spec 和 module
    ...
    def test_get_ms_ops_info_without_module_file_fallback(self):
        """_get_ms_ops_info 无 module_file 时回退到 import_module（向后兼容）"""
        mock_module = mock.MagicMock()
        mock_module.test_attr = "test_value"
